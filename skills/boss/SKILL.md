...
8. **Report** — compact: what changed, lanes used, evidence, judgment calls, open items. A new durable signal from this run (a lane override, effort dial, style correction, a tool the user lacks, a repo convention a builder surfaced under DEVIATIONS or OPEN) gets saved per the Memory rule above; mention it in at most one clause, e.g. "(noted: keep reports short)" — never a section.
